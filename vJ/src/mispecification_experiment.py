...
from ground_truth import grid_search, get_lastfm_ground_truths, get_movielens_ground_truths

def get_model_envy(policies, ground_truths, epsilon=0.05):
    """
    Computes the average envy of users and the proportion of epsilon-envious
    users in a system given recommendation probabilities and ground truth
    preferences.
    Inputs:
        policies - Matrix with probabilities of items (columns) being
                   recommended to users (rows)
        ground_truth - Matrix with ground truth preferences of user (rows) for
                       items (columns)
    ...
    """
    assert policies.shape == ground_truths.shape, \
           f"Shapes {policies.shape} and {ground_truths.shape} do not match."
    user_envies = np.zeros(policies.shape[0])
    for target_user in range(policies.shape[0]):
        max_util = 0
        utils = np.sum(policies * ground_truths[target_user], axis=1)
...
def do_experiment(ground_truths, factors, seed=None):
    ...
    for factor in factors:
        ...
        hyperparams = grid_search(train_matrix, val_matrix, [factor], reg,
                                  conf_weights, seed)
        ...
        # Assess envy
        avg_envy, prop_envious = get_model_envy(rec.policies, ground_truths, epsilon)
        avg_envy_list.append(avg_envy)
        prop_envious_list.append(prop_envious)
    return avg_envy_list, prop_envious_list
        
if __name__ == '__main__':
    start = time.perf_counter()
    gt = get_movielens_ground_truths()
    factors = [1, 2, 4, 8, 16, 32, 64, 128, 256]
    avg_envy, prop_envious = do_experiment(gt, factors, 42)
